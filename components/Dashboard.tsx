...
export const Dashboard: React.FC<DashboardProps> = ({ onLoadProfile, onNewAssessment }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-900">Your Career Profiles</h2>
          <p className="text-slate-500 mt-1">Track your AI impact assessments over time.</p>
        </div>
        <button
      ...
      {profiles.length > 1 && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="font-bold text-slate-800">Impact Trend</h3>
          </div>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="automate" stroke="#ef4444" fillOpacity={1} fill="url(#colorAuto)" strokeWidth={2} name="Automate" />
                <Area type="monotone" dataKey="augment" stroke="#3b82f6" fillOpacity={1} fill="url(#colorAug)" strokeWidth={2} name="Augment" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {profiles.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-300">
          ...
          <h3 className="text-xl font-semibold text-slate-700">No saved profiles yet</h3>
          <p className="text-slate-500 mt-2 mb-6">Start your first assessment to see it here.</p>
          ...
          {profiles.map((profile) => (
            ...
            >
                ...
                <div className="mt-4 flex gap-3">
                    {profile.hardSkills?.length > 0 && (
                        <div className="flex items-center gap-1 text-xs text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md">
                            <Brain size={12} />
                            <span className="font-medium">{profile.hardSkills.length} Skills</span>
                        </div>
                    )}
                    {profile.softSkills?.length > 0 && (
                        <div className="flex items-center gap-1 text-xs text-pink-600 bg-pink-50 px-2 py-1 rounded-md">
                            <Heart size={12} />
                            <span className="font-medium">{profile.softSkills.length} Soft Skills</span>
                        </div>
                    )}
                </div>
              )}
              ...
              <div className="mt-auto pt-4 border-t border-slate-100 flex justify-between items-center text-sm text-slate-500">
                <span>{profile.tasks.length} tasks analyzed</span>
